...
static void alds_log_info(const char * const msg, va_list va){
    assert_string_equal(msg, "INFO[Module 1]: general_log_test 1");
    call_index = 1; 
}

static void alds_log_warning(const char * const msg, va_list va){
    assert_string_equal(msg, "WARNING[Module 2]: general_log_test 2");
    call_index = 2; 
}

static void alds_log_error(const char * const msg, va_list va){
    assert_string_equal(msg, "ERROR[Module 3]: general_log_test 3");
    call_index = 3; 
}

static void alds_log_module_len_limit(const char * const msg, va_list va){
    assert_string_equal(msg, "ERROR[VeryLongModuleLimit12]: general_log_test 4");
    call_index = 4; 
}

static void alds_log_module_len_cut(const char * const msg, va_list va){
    assert_string_equal(msg, "ERROR[VeryLongModuleLimi...]: general_log_test 5");
    call_index = 5; 
}

static void alds_log_module_va_arg(const char * const msg, va_list va){
    assert_string_equal(msg, "INFO[Module]: Some module");
...
static void general_log_test(void ** state) {
    ...
    ALDS_LOG_ERROR("VeryLongModuleLimit12", "general_log_test 4");
    assert_int_equal(call_index, 4);

    alds_set_log_cb(alds_log_module_len_cut);
    ALDS_LOG_ERROR("VeryLongModuleLimit123", "general_log_test 5");
