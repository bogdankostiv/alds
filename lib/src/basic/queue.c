...
alds_err_t alds_queue_init(alds_queue_t * ctx, alds_memmngr_t * memmngr,
    ...
    ctx->buff =
        alds_memmngr_buffer_get(&ctx->mem_mngr, items_quantity * item_size);
    if (NULL == ctx->buff) {
...
alds_err_t alds_queue_push(alds_queue_t * ctx, const void * data) {
    ...
    memcpy(ctx->buff + ctx->item_index_head * ctx->item_size, data,
           ctx->item_size);
...
alds_err_t alds_queue_pop(alds_queue_t * ctx, void * data) {
    ...
    ctx->items_quantity_curr--;
    memcpy(data, ctx->buff + ctx->item_index_tail * ctx->item_size,
           ctx->item_size);
