...
#define ALDS_DATA_INIT_LOCAL(data_name, data_size)                             \
    ...
    alds_data_t data_name = {                                                  \
        .ptr = (void *)data_name##buff, .size = data_size, .free_cb = NULL}
...
#define ALDS_DATA_INIT_CUSTOM(data_name, data_ptr, data_size, free_cb_in)      \
    alds_data_t data_name = {                                                  \
        .ptr = data_ptr, .size = data_size, .free_cb = free_cb_in}
...
#define ALDS_DATA_PTR_IS_VALID(data)                                           \
    ((NULL == data || NULL == (data)->ptr || 0 == (data)->size) ? false : true)
